...
vector<int> read_outliers(const string& file) {
  vector<int> res;
  ifstream fin(file);
  char ch;
  int idx;
...
map<string, double> run_exp(const mpi::communicator& world,
  ...
  for (int ith = 0; ith < n_repeats; ++ith) {
    ...
    if (mode == "bg") {
      ...
      mpi_kmean(world, ROOT, U, coreset, coreset_size / 4.5, msg_size);
      msg_size *= world.size();
    } else {
...
bool parse_args(int argc, char* argv[],
  ...
  if (modes.count("rand") > 0 || modes.count("kmean++") > 0
      || modes.count("km") > 0 || modes.count("kmean--") ) {
    coreset_size = atoi(argv[i++]);
  }
  dataset = argv[i++];
  outliers_file = argv[i++];
  if (i >= argc) {
    n_repeats = 1;
  } else {
...
int main(int argc, char* argv[]) {
  ...
  string dataset;
  set<string> modes;
  if (!parse_args(argc, argv, modes, n_centers, n_outliers, coreset_size, dataset, outliers_file, n_repeats)) {
    if (world.rank() == ROOT) {
      cerr << "usage: mpirun -np n_partitions ./mpi_exp2 [bg|bg2|km|rand|kmean++|kmean--]+ k t [coreset_size] dataset outliers_file [n_repeats = 1]" << endl;
    }
    return 0;
  }
  ...
  if (world.rank() == ROOT ) {
    ...
    dataSet.load();
    actual_outliers = read_outliers(outliers_file);
    cerr << "|outliers| = " << actual_outliers.size() << endl;
